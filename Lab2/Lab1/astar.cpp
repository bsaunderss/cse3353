...
using namespace std;
void astar::search(graph& adjList, int src, int dst){
    ...
    while(destFound == false && done == false){
        if(weight_vertex_path.size()== 0){
            ...
            for(int i = 0; i<children.size(); i++){
                ...
                if(children[i] == dst){
                    //changing getWeight to getDistance
                    weight = get<0>(weight_vertex_path[0]) + graphdata.getCost(parent, children[i]);
...
bool astar::alreadyVisited (int v, std::vector<int>visited){
...
bool astar::onPriority (int vertex, std::vector<std::tuple<int, int, int>> wvf){
    bool itsThere = false;
...
int astar::spotOnPriority (int vertex, std::vector<std::tuple<int, int, int>> wvf){
    int spot;
    for(int i = 0; i<wvf.size(); i++){
        if(get<1>(wvf[i]) == vertex){
            spot = i;
